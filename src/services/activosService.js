...
export async function createAsset(nombre, categoria, estado) {
    ...
    try {
        ...
        return {
            id: res.rows[0].id,
            nombre,
            categoria: res.rows[0].categoria,
...
export async function updateAsset(id, nombre, categoria, estado) {
    ...
    try {
        ...
        // Verify existence
        const checkRes = await client.query("SELECT id FROM activos WHERE id = $1", [id]);
        ...
        const queryText = `
            ...
            RETURNING id, categoria, estado, created_at, updated_at
        `;
        const res = await client.query(queryText, [nombre, ENCRYPTION_KEY, categoria, estado, id]);
